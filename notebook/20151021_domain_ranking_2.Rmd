---
title: "General overlap in features affected by mutations and switches 2"
...
wd <- paste0("~/SmartAS/Pipeline/notebook/20151021_domain_ranking_2/data/")
out <- paste0("~/SmartAS/Pipeline/notebook/20151021_domain_ranking_2/tables/")
```
...
* Domain frequency: frequency of a feature in the proteome, counting the features in the most abundant isoform for each gene. Used to normalize the previous quantity.
...
* Domain frequency, for mutation purposes: frequency of a feature in the proteome, as the accumulated length in the most abundant isoform for each gene. Used to normalize the previous quantity.
...
```{r tumor_mutVsSwitch}
feat_enrich <- read.delim(paste0(wd,'feature_enrichment.txt'),row.names=NULL)
...
feat_enrich$DomainSizeFrequency <- feat_enrich$DomainSize/feat_enrich$TotalProteomeSize
...
for (cancer in cancerTypes){
  ...
  minMut <- quantile(cancer.feat_enrich$MutFreq/cancer.feat_enrich$DomainSizeFrequency,0.95)
  minSwitch <- quantile(cancer.feat_enrich$SwitchFreq/cancer.feat_enrich$DomainFrequency,0.95)
  
  # plot raw data
  r <- cor(cancer.feat_enrich$MutFreq/cancer.feat_enrich$DomainSizeFrequency,
           cancer.feat_enrich$SwitchFreq/cancer.feat_enrich$DomainFrequency)
  
  p <- ggplot(cancer.feat_enrich,aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency)) + 
    geom_point() + 
    geom_point(data=subset(cancer.feat_enrich, MutFreq/DomainSizeFrequency > minMut & SwitchFreq/DomainFrequency > minSwitch),aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency,color=Domain)) + 
    smartas_theme() + 
...
for (cancer in cancerTypes){
  ...
  r <- cor(r.df$MutFreq/r.df$DomainSizeFrequency,r.df$SwitchFreq/r.df$DomainFrequency)
  
  p <- ggplot(r.df,aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency)) + 
    geom_point() + 
    geom_point(data=subset(r.df, MutFreq/DomainSizeFrequency > minMut & SwitchFreq/DomainFrequency > minSwitch),aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency,color=Domain)) + 
    smartas_theme() + 
    geom_smooth(method=lm) + 
...
feat_enrich_agg <- ddply(feat_enrich,.(Domain), summarise, 
                        ...
                        DomainSize=sum(DomainSize),TotalProteomeSize=sum(TotalProteomeSize))
...
feat_enrich_agg$DomainSizeFrequency <- feat_enrich_agg$DomainSize/feat_enrich_agg$TotalProteomeSize

minMut <- quantile(feat_enrich_agg$MutFreq/feat_enrich_agg$DomainSizeFrequency,0.95)
minSwitch <- quantile(feat_enrich_agg$SwitchFreq/feat_enrich_agg$DomainFrequency,0.95)

# plot original data
r <- cor(feat_enrich_agg$MutFreq/feat_enrich_agg$DomainSizeFrequency,
         feat_enrich_agg$SwitchFreq/feat_enrich_agg$DomainFrequency)

p <- ggplot(data=feat_enrich_agg,
            aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency)) + 
  geom_point() + 
  geom_point(data=subset(feat_enrich_agg,
                         MutFreq/DomainSizeFrequency > minMut & 
                         SwitchFreq/DomainFrequency > minSwitch),
             aes(MutFreq/DomainSizeFrequency,
...
p <- ggplot(data=r.df,aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency)) + 
  geom_point() + 
  geom_point(data=subset(r.df,MutFreq/DomainSizeFrequency > minMut & 
                         SwitchFreq/DomainFrequency > minSwitch),
             aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency,color=Domain)) + 
  smartas_theme() + 
  geom_smooth(method=lm) + 
  geom_text(x=6,y=20,label=paste0("R = ",round(r,2)))
p <- direct.label(p)
...
for (cancer in cancerTypes){
  ...
  # plot normalized mutation frequency and switched frequency
  mutOutliers <- quantile(cancer.feat_enrich$MutFreq/cancer.feat_enrich$DomainSizeFrequency,0.95)
  switchOutliers <- quantile(cancer.feat_enrich$SwitchFreq/cancer.feat_enrich$DomainFrequency,0.95)
  
  # remove outliers
  cancer.feat_enrich <- cancer.feat_enrich[cancer.feat_enrich$MutFreq/cancer.feat_enrich$DomainSizeFrequency <= mutOutliers,]
...
for (cancer in cancerTypes){
  ...
  cancer.feat_enrich$p_mut <- apply(cancer.feat_enrich[,c("MutIn","AllMuts","DomainSizeFrequency")],1, function(x){ 
...
feat_enrich_agg.enrichment$p_mut <- apply(feat_enrich_agg.enrichment[,c("MutIn","AllMuts","DomainSizeFrequency")],1, function(x){ 
  p <- binom.test(x[1],x[2],x[3],"greater")
  p$p.value
} )
...
r <- cor(df$MutFreq/df$DomainSizeFrequency,df$SwitchFreq/df$DomainFrequency)
  
ggplot(df,aes(MutFreq/DomainSizeFrequency,SwitchFreq/DomainFrequency)) + 
  geom_point() + 
  smartas_theme() + 
  geom_smooth(method=lm) + 
  geom_text(x=4,y=50,label=paste0("R = ",round(r,2)))
...
df <- feat_enrich_agg.enrichment[order(feat_enrich_agg.enrichment$p_mut),c("Domain","adjp_mut","adjp_swt","MutFreq","SwitchFreq","DomainFrequency","DomainSizeFrequency")]
...
df$fc_mut <- round(df$MutFreq/df$DomainSizeFrequency,2)
