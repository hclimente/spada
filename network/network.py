...
class Network:
	...
	def add_node(self, full_name="", score=0.0, gene_id="?", gene_symbol="?", switch=False, driver=False):
		...
		self._net.add_node( geneID, score=Score, symbol=geneSymbol, switch=switch, driver=driver )
	...
	def importCandidates(self, path, samples):
		...
		candidates = candidates[ ["Gene", "Replicates"] ]
		
		#Remove less significant switches
		candidates = candidates[ candidates.Replicates >= min_samples ]
		candidates = candidates.groupby("Gene").sum()
		candidates.Replicates = 0.2 + 0.3 * (candidates.Replicates - min_samples)/(samples - min_samples)
		
		for gene in candidates.index.values:
