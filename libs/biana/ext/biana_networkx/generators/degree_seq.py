...
import random
import biana_networkx as networkx
import biana_networkx.utils
from biana_networkx.generators.classic import empty_graph
import heapq
#---------------------------------------------------------------------------
...
def is_valid_degree_sequence(deg_sequence):
    ...
    if not biana_networkx.utils.is_list_of_ints(deg_sequence):
...
def double_edge_swap(G, nswap=1):
    ...
    cdf=biana_networkx.utils.cumulative_distribution(deg.values())  # cdf of degree
    if len(cdf)<4:
        raise networkx.NetworkXError, \
...
#        if random.random() < 0.5: continue # trick to avoid periodicities?
        ...
        (ui,xi)=biana_networkx.utils.discrete_sequence(2,cdistribution=cdf) 
...
def connected_double_edge_swap(G, nswap=1):
    ...
    dk=deg.keys() # key labels 
    ideg=G.degree()
    cdf=biana_networkx.utils.cumulative_distribution(G.degree()) 
    if len(cdf)<4:
        raise networkx.NetworkXError, \
          "Graph has less than four nodes."
    window=1
